...
import { getProductById, getProducts, updateCart } from "../services/api";
...
export default function ProductDetailPage() {
  ...
  // ✅ FIXED: No fetch, no nesting, clean API flow
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        const data = enrich(res.data);

        setProduct(data);

        // Get related products via centralized API
        const all = await getProducts();

        const relatedItems = all.data
          .filter(
            (p) =>
  ...
  // ✅ FIXED: Uses API service instead of fetch + token manual handling
  ...
  // ⚠️ Fake review API (kept same behavior, cleaned architecture)
  const handleReview = async (e) => {
    ...
    try {
      // Fake API simulation via same service layer pattern
      await updateCart({
        productId: id,
        rating: 5,
      });
        ...
        <div className="md:col-span-2 flex flex-col gap-5">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <code className="text-sm text-gray-800 font-mono">
              npm install{" "}
              {product.title.split(" ")[0].toLowerCase()}-sdk
            </code>
          </div>
